...
from backend.ai.contextual_prompt import (
    ...
    generate_contextual_fallback_advice,
...
class AIClient:
    ...
    def _generate_local_fallback_advice(self, prompt: str) -> str:
        ...
        fallback_response = {
            ...
            "risk_warning": "All investments carry risk, including potential loss of principal. Past performance does not guarantee future results. Consider consulting a financial advisor before making investment decisions.",
            "assumptions": {
...
class ContextualAIClient(AIClient):
    """AI client optimized for contextual, multi-area financial advice.
    
    Extends base AIClient with contextual prompt building that integrates
    all financial data (profile, metrics, spending, goals, portfolio) into
    comprehensive, personalized advice across 4 key areas:
    - Budget optimization
    ...
    def generate_contextual_advice(
        self,
        report: Dict[str, Any],
    ) -> Dict[str, Any]:
        """Generate comprehensive contextual financial advice.
        
        Uses complete financial report to build a detailed Gemini prompt that
        covers all aspects of the user's financial situation and provides
        structured advice across 4 key areas.
        
        Args:
            report: Complete financial report from build_user_report()
                    Must contain: user_profile, financial_snapshot,
                    computed_metrics, holdings_summary, goals_analysis,
                    spending_analysis, advanced_spending_analysis,
                    goal_planning, portfolio_recommendation
        
        Returns:
            dict: Validated advice response with keys:
                - budget_advice: List of budget improvement recommendations
                - debt_strategy: Debt repayment strategy and actions
                - investment_strategy: Investment diversification recommendations
                - goal_roadmap: Step-by-step goal achievement roadmap
                - risk_explanation: Risk factors and important disclaimers
                - human_summary: Plain language summary for non-experts
        
        Raises:
            ValueError: If report is missing required sections
        """
        ...
        try:
            ...
            logger.error(f"Invalid report for user {user_id}: {e}")
            raise
        ...
        if cached:
            ...
            model_used = "cache"
            model_version = "cached"
            logger.info(f"Cache hit for contextual advice, user {user_id}")
        else:
            ...
            if self.has_gemini:
                try:
                    ...
                    model_used = "gemini"
                    model_version = "gemini-pro"
                    logger.info(f"Gemini contextual advice generated for user {user_id}")
                except Exception as e:
                    logger.error(f"Gemini contextual call failed: {e}. Using fallback.")
                    response_json = json.dumps(
                        generate_contextual_fallback_advice(report)
                    )
                    model_used = "local-fallback"
                    model_version = "fallback-contextual-v1"
            else:
                logger.info(f"No Gemini available, using fallback for user {user_id}")
                response_json = json.dumps(
                    generate_contextual_fallback_advice(report)
                )
                model_used = "local-fallback"
        ...
        # Validate response
        validated_response, validation_error = validate_contextual_output(response_json)
        
        if validation_error:
            logger.warning(f"Contextual response validation failed: {validation_error}")
            # Use fallback
            validated_response = generate_contextual_fallback_advice(report)
            model_used = "local-fallback"
            model_version = "fallback-contextual-v1"
            validation_error = f"Validation failed, fallback used: {validation_error}"
...
def generate_advice(
    ...
    """Convenience function to generate template-based advice.
...
def generate_contextual_advice(
    ...
    """Convenience function to generate comprehensive contextual advice.
    
    Uses complete financial data to provide personalized advice across
    ...
    Returns:
        dict: Contextual advice response with budget_advice, debt_strategy,
              investment_strategy, goal_roadmap, risk_explanation, human_summary
    """
    client = ContextualAIClient()
    return client.generate_contextual_advice(report)
