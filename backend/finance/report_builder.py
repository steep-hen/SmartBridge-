...
def build_user_report(
    ...
    spending_analysis = _build_spending_analysis(
        transactions, latest_summary
    )
    
    advanced_analysis = _build_advanced_spending_analysis(
        transactions, latest_summary, user
    )
    
    goal_planning = _build_goal_planning(
        goals, latest_summary
    )
    
    portfolio_recommendation = _build_portfolio_recommendation(
        user, latest_summary, holdings
    )
    ...
    report = {
        ...
        'advanced_spending_analysis': advanced_analysis,
...
def _build_spending_analysis(
    transactions: List[Transaction],
    summary: Optional[FinancialSummary],
) -> Dict[str, Any]:
    """Build spending analysis section using transaction data."""
    if not summary or not transactions:
        return {
            'available': False,
            'budget_ratio': 0.0,
            'budget_status': 'UNKNOWN',
            'budget_status_description': 'Insufficient data',
    ...
    budget_status = categorize_budget_ratio(budget_ratio)
    
    # Calculate category spending distribution
    category_distribution = calculate_category_spending_distribution(
        transactions, monthly_income
    )
    
    # Detect high spending categories
    high_spending_alerts = detect_high_spending_categories(
        category_distribution, monthly_income
    )
    
    # Detect recurring subscriptions
    recurring_subscriptions = detect_recurring_subscription_charges(transactions)
    
    # Generate recommendations
    recommendations = generate_spending_recommendations(
        category_distribution, high_spending_alerts, recurring_subscriptions, budget_ratio
    )
    ...
    return {
        ...
        'budget_status_description': budget_status['description'],
        'budget_status_color': budget_status['color'],
        'category_distribution': category_distribution,
        'high_spending_alerts': high_spending_alerts,
        'recurring_subscriptions': recurring_subscriptions,
        'recommendations': recommendations,
    }


def _build_advanced_spending_analysis(
    transactions: List[Transaction],
    summary: Optional[FinancialSummary],
    user: Optional[User],
) -> Dict[str, Any]:
    """Build advanced spending analysis section."""
    if not summary or not transactions:
        return {
            'available': False,
            'timestamp': None,
    ...
    advanced = generate_advanced_spending_analysis(
        transactions, summary, user_country
    )
    
    return advanced


def _build_goal_planning(
    goals: List[Goal],
    summary: Optional[FinancialSummary],
) -> Dict[str, Any]:
    """Build goal planning strategies for all active goals.
    
    Generates savings strategies for each goal using goal_planner module.
    
    Args:
        goals: List of Goal objects from database
        summary: Latest financial summary with income/expenses
    
    Returns:
        Dict with goal strategies, feasibility assessment, and combined recommendations
    """
    if not goals or not summary:
        return {
            'available': False,
            'total_goals': 0,
            'goal_strategies': [],
    ...
    monthly_income = float(summary.total_income) / max(summary.month, 1)
    monthly_expenses = float(summary.total_expenses) / max(summary.month, 1)
    current_savings = float(summary.total_savings) if summary.total_savings else 0.0
    ...
    for goal in goals:
        ...
        if strategy.get('available'):
            ...
            for rec in strategy.get('strategy', []):
                combined_recommendations.add(rec)
    
    return {
        'available': True,
        'total_goals': len(goal_strategies),
        'goal_strategies': goal_strategies,
        'combined_recommendations': sorted(list(combined_recommendations)),
...
def _build_portfolio_recommendation(
    ...
    """Build portfolio recommendation section.
    
    Generates portfolio allocation recommendation based on user profile
    and financial situation using portfolio_optimizer module.
    
    Args:
        user: User object with profile information
        summary: Latest financial summary
        holdings: List of user's current holdings
    
    Returns:
        Dict with recommended portfolio allocation and instruments
    """
    ...
    # Determine risk tolerance - default to medium if not available
    # Can be extended if user model includes risk_tolerance field
    ...
    try:
        recommendation = generate_portfolio_recommendation(
            ...
            age=age,
            monthly_investment=monthly_surplus if monthly_surplus > 0 else None,
        )
        
        recommendation['available'] = True
        recommendation['monthly_investment_capacity'] = _round_currency(monthly_surplus)
        recommendation['current_portfolio_summary'] = {
        ...
        # If recommendation generation fails, return unavailable
        return {
            'available': False,
            'timestamp': None,
        }
