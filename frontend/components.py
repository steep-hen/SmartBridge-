...
def chart_holdings_allocation(holdings: List[Dict[str, Any]]):
    ...
    Args:
        holdings: List of {'ticker', 'current_value', ...}
    """
    if not holdings:
    ...
    if total_value == 0:
        st.info("No portfolio value to display")
        return
    
    # Build labels and values with allocation percentages
    labels = []
    values = []
    ...
    for h in holdings:
        ...
        labels.append(f"{ticker} ({allocation_pct:.1f}%)")
        values.append(value)
